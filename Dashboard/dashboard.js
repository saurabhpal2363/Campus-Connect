...
document.querySelector(".u-name").textContent = data.name;
document.querySelector(".u-dept").textContent =
...
const si =
document.querySelector(`.sidebar .nav-item[data-page="${page}"]`);
...
const bi =
document.querySelector(`.bn-item[data-page="${page}"]`);
...
const wasPlaying =
card.classList.contains('now-playing');
...
const btn =
document.getElementById('prb-'+id);
...
const idx =
cards.findIndex(c=>c.dataset.vid===String(id));
...
if(e.isIntersecting &&
!document.querySelector('.reel-card.now-playing')){
...
const n =
parseInt(btn.textContent.replace(/\D/g,''));

btn.textContent =
(liked?'❤️ ':'🤍 ')+(liked?n+1:n-1);
...
el.closest('.feed-tabs')
.querySelectorAll('.f-tab')
...
el.closest('.network-tabs')
.querySelectorAll('.n-tab')
.forEach(t=>t.classList.remove('active'));
...
document.querySelectorAll('.i-chip')
...
document.getElementById('uzSub').textContent=
...
document.querySelectorAll('.conv-row')
.forEach(r=>r.classList.remove('active'));

row.classList.add('active');

document.getElementById('chatAv').textContent=initials;
document.getElementById('chatAv').style.background=
...
div.innerHTML =
text + '<span class="msg-time">Just now</span>';
...
reply.innerHTML =
'That sounds great! 👍<span class="msg-time">Just now</span>';
