...
window.addEventListener("DOMContentLoaded", () => {
  ...
  const storedName   = localStorage.getItem("cc_name");
  const storedBranch = localStorage.getItem("cc_branch");
  const storedYear   = localStorage.getItem("cc_year");

  if (storedName) {
    const nameEl = document.querySelector(".u-name");
    const deptEl = document.querySelector(".u-dept");
    if (nameEl) nameEl.textContent = storedName;
    if (deptEl) deptEl.textContent = (storedBranch || "—") + " · " + (storedYear || "—");
  }
...
function tickProgress(id) {
  ...
  playIntervals[id] = setInterval(() => {
    pct += 0.12;
    if (pct >= 100) { pct = 0; clearInterval(playIntervals[id]); autoAdvance(id); }
    bar.style.width = pct + '%';
  }, 80);
}
...
function autoAdvance(id) {
  ...
  if (next) { next.scrollIntoView({ behavior:'smooth', block:'nearest' }); setTimeout(() => toggleReel(next), 400); }
}
...
function goEvent(btn) { btn.classList.add('going'); btn.textContent = '✓ Going'; }
...
function fileSelected(input) {
  if (input.files && input.files[0]) {
    ...
    document.getElementById('uzTitle').textContent = input.files[0].name;
...
function openChat(row, name, initials, gradient, statusText) {
  ...
  document.getElementById('chatStatus').textContent =
    statusText === 'Online' ? '● Online' : statusText === 'Away' ? '● Away' : '○ Offline';
}
